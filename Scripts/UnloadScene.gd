...
func clear_nodegame_except_spawner():
	if not is_instance_valid(GameController.nodegame):
		return
	
	for child in GameController.nodegame.get_children():
		if child.name != "LevelSpawner": # ⚡ aquí pon el nombre exacto de tu spawner en la escena
			child.queue_free()
